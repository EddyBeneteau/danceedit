const data = {
  fr: {
    title: "Montage musical pour danseurs | DanceEdit Studio",
    ...
    basicItems: ["1 morceau","Durée exacte","Transitions propres","Coupe musicale optimisée pour compétition"],
    perfItems: ["1 à 2 morceaux","Transitions fluides","Montée d’énergie et effets si nécessaire","Montage prêt pour compétition"],
    premiumItems: ["Montage créatif","Mashup (à partir de plusieurs morceaux)","Composition rythmique et effets si nécessaire","Remplacement de pistes ou ajout d’harmoniques"],
    customItems: ["Projet complexe ou spécifique","Commande pour plusieurs montages","Délais urgents","Tarif personnalisé"],
    about: "À propos",
  ...
  en: {
    title: "Dance Music Editing for Dancers | DanceEdit Studio",
  ...
  es: {
    title: "Edición musical para bailarines | DanceEdit Studio",
...
let lang = (navigator.language || "fr").toLowerCase().startsWith("es")
  ? "es"
  : (navigator.language || "fr").toLowerCase().startsWith("fr") ? "fr" : "en";

let currency = (() => {
  const loc = (Intl.NumberFormat().resolvedOptions().locale || "").toLowerCase();
